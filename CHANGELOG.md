...
- Credential Profiles: backend foundation for per-project and per-thread AI credential overrides. Adds a global `credential-profiles.json` store, IPC handlers (`credentials:list / :create / :update / :delete / :references`), and a new resolution chokepoint (`resolveCredential`) with precedence session → project profile → project legacy `apiKey` → global. Explicit profiles bypass the claude-code authMethod gate so a per-project API key works even when the user is logged in via OAuth globally. (fork-local)
- Credential Profiles UI: new Settings → Credentials → Credential Profiles panel (per-provider list, create / edit / delete, masked key with reveal, refused-delete modal listing referencing projects + threads, opt-in "create profile from existing global key" affordance). Project AI Providers panel replaces the raw API key input with a profile picker dropdown plus a one-time "convert legacy saved key into a profile" prompt. Per-thread credential picker appears next to the model selector in the chat input — a quiet key icon when inheriting, a labeled pill when the thread has an explicit override. (fork-local)
